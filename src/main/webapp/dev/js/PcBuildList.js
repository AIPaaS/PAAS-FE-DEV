	...
	cb : function(r) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (data[i].lastBuildTask.status == "2") {
			    $("#thisBackBuildStatues_" + data[i].def.id).val(
				    "2");
			    $("#thisBackBuildId_" + data[i].def.id).val(
				    data[i].lastBuildTask.backBuildId);
			    $("#a_build_task_gj_" + data[i].def.id).css(
				    "display", "none");
			    $("#a_build_task_zd_" + data[i].def.id).css(
		    ...
		    $("#a_build_task_gj_" + data[i].def.id).bind(
				...
				var obj = CurrDataMap["key_"
					+ this.id.substring(this.id
						.lastIndexOf("_") + 1)];
				gj_BuildDef(obj.def.id, obj.def.depTag,
					obj.def.imageDefId, obj.def.buildName,
					obj.imageDef.imageFullName);
			    });
...
function queryBuildTaskRecord() {
	...
	cb : function(data) {
		...
		$("#prod_proj").text(
			CurrObj.product.code + '|' + CurrObj.project.code);
		CurrTask = data[0];
		$("#stdoutList").empty();
	...
	RS.ajax({
		...
		if (!CU.isEmpty(data)) {
			...
			if (data.building == false) {
			    ...
			    setTimeout('queryTaskRecord()', 5000);
			}
...
function searchTaskStatus(id){
	...
	success:function(data){
		...
		$("#td_build_task_msage_"+id).text(PU.getDropValue("V_PC_BUILD_TASK_STATUS",obj.status,false));
		$("#td_build_task_startTime_"+id).text("");
		$("#td_build_task_startTime_"+id).text(CU.toStringDateTime(obj.taskStartTime));
		$("#td_build_task_buildId_"+id).text("");
		$("#td_build_task_buildId_"+id).text(obj.backBuildId);
